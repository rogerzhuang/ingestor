...
class EDGARUpdater(Updater):
    def __init__(self, start_date, root_folder, updater_params):
        super().__init__(start_date, root_folder)
        self.form_types = updater_params.get('form_types')
        self.symbols = updater_params.get('symbols')

    def load_cik_pairs(self, filename):
        cik_dict = {}
    ...
    def update(self):
        cik_pairs = self.load_cik_pairs('./updaters/cik.csv')
        dl = Downloader()
        for form_type in self.form_types:
            for symbol in self.symbols:
                ...
                dl.get(form_type, cik, after=self.start_date)
                ...
                if os.path.exists(folder_path):
                    for folder_name in os.listdir(folder_path):
                        ...
                        if os.path.exists(html_file):
                            ...
                            new_folder = f'{self.root_folder}/EDGAR/{symbol}/{form_type}/{report_date}'
                    ...
                    print(
                        f"Folder path {folder_path} does not exist. Skipped.")
